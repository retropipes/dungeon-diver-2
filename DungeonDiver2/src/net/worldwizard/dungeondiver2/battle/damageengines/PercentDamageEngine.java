...
public class PercentDamageEngine extends DamageEngine {
	...
	if (this.fumble(acting)) {
	    ...
	    this.missed = false;
	    this.dodged = false;
	    final RandomRange fumDamage = new RandomRange(1, Math.max(acting.getUnfactoredWeaponPower(), 1));
	    return -fumDamage.generate();
	} else {
	    final double attack = acting.getEffectedUnfactoredAttack();
	    final double block = enemy.getEffectedStat(StatConstants.STAT_BLOCK);
	    final double absorb = (PercentDamageEngine.ABSORB - enemy.getArmorBlock()) / PercentDamageEngine.ABSORB;
		...
		if (rEvade < aEvade) {
		    ...
		    this.missed = false;
		    this.dodged = false;
		    final RandomRange rDamage = new RandomRange(PercentDamageEngine.MULTIPLIER_MIN,
	...
	return fumChance <= fumbler.getEffectedStat(StatConstants.STAT_FUMBLE_CHANCE);
    }
