...
public class BattleGUI {
	...
	for (int x = 0; x < friends.length + this.hostileCount; x++) {
	    ...
	    this.bd.getBattlerAIContexts()[x] = new AIContext(this.bd.getBattlers()[x], this.bd.getBattleMap());
	}
	...
	if (!this.resultDoneAlready) {
		...
		CommonDialogs.showTitledDialog("The party gains " + exp + " experience and " + gold + " Gold.",
			"Victory!");
		PartyManager.getParty().distributeVictorySpoils(exp, gold);
		DungeonDiver2.getApplication().getGameManager().addToScore(Math.max(1, (exp + gold) / (100
			* (PartyManager.getParty().getActivePCCount() + PartyManager.getParty().getActiveNPCCount()))));
	    ...
	    // Level Up Check
	    final ArrayList<GameScript> scripts = PartyManager.getParty().checkPartyLevelUp();
	...
	try {
		...
		final int x = this.bd.getActiveCharacter().getTemplate().getAI().getMoveX();
		final int y = this.bd.getActiveCharacter().getTemplate().getAI().getMoveY();
		this.lastAIActionResult = this.updatePosition(x, y);
		this.bd.getActiveCharacter().getTemplate().getAI().setLastResult(this.lastAIActionResult);
		break;
	...
	if (this.fumbleDamage != 0) {
	    displayDamageString = "FUMBLE! " + activeName + " drops their weapon, doing " + fumbleDamageString
		...
		if (this.de.weaponMissed()) {
		    ...
		    SoundManager.playSound(GameSounds.ATTACK_MISS);
		} else if (this.de.enemyDodged()) {
		    displayDamageString = activeName + " tries to hit " + enemyName + ", but " + enemyName
			    + " AVOIDS the attack!";
		    SoundManager.playSound(GameSounds.MISSILE_DODGE);
		} else {
		    displayDamageString = activeName + " tries to hit " + enemyName + ", but MISSES!";
		    SoundManager.playSound(GameSounds.ATTACK_MISS);
		}
	    } else {
		displayDamageString = activeName + " hits " + enemyName + " for " + damageString + " damage!";
	...
	for (int x = 0; x < this.speedArray.length; x++) {
		...
		this.speedArray[x] = this.bd.getBattlers()[x].getTemplate().getEffectedSpeed();
	    } else {
	...
	final RandomRange randX = new RandomRange(0, this.bd.getBattleMap().getRows() - 1);
	final RandomRange randY = new RandomRange(0, this.bd.getBattleMap().getColumns() - 1);
	int rx, ry;
	// Set Character Locations
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlers()[x].isActive()) {
		    rx = randX.generate();
		    ry = randY.generate();
		    MapObject obj = this.bd.getBattleMap().getBattleCell(rx, ry);
		    ...
		    this.bd.getBattleMap().setBattleCell(this.bd.getBattlers()[x], rx, ry);
		}
	    }
	}
    }
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlers()[x].getTeamID() != 0) {
		    res += this.bd.getBattlers()[x].getTemplate().getExperience();
		}
	    }
	}
	...
	if (next != null && nextGround != null) {
		...
		if (useAP && this.getActiveActionCounter() >= nextGround.getBattleAPCost() || !useAP) {
		    // Move
		    MapObject obj1 = null;
		    MapObject obj2 = null;
		    MapObject obj3 = null;
		    ...
		    this.decrementActiveActionCounterBy(nextGround.getBattleAPCost());
		...
		if (next.isOfType(TypeConstants.TYPE_BATTLE_CHARACTER)) {
			...
			if (!enemy.isAlive()) {
			    ...
			    this.bd.getBattleMap().setBattleCell(new Empty(), bc.getX(), bc.getY());
			}
			// Handle self death
			if (!active.getTemplate().isAlive()) {
			    ...
			    active.deactivate();
			    // Remove character from battle
			    this.bd.getBattleMap().setBattleCell(new Empty(), active.getX(), active.getY());
			    // End turn
			    this.endTurn();
			}
		    } else {
		    ...
		    SoundManager.playSound(GameSounds.HIT_OBSTACLE);
		    return false;
		}
	    }
	} else {
	...
	if (this.bossMode) {
	    if (PreferencesManager.getMusicEnabled(PreferencesManager.MUSIC_BOSS)) {
	...
	this.battleFrame.setJMenuBar(DungeonDiver2.getApplication().getMenuManager().getMainMenuBar());
    }
	...
	if (this.getActiveSpellCounter() > 0) {
		...
		final Spell sp = this.bd.getActiveCharacter().getTemplate().getAI().getSpellToCast();
	...
	if (this.getActiveActionCounter() > 0) {
	    if (!this.bd.getActiveCharacter().getTemplate().hasAI()) {
		// Active character has no AI
		final boolean success = CombatItemManager.selectAndUseItem(this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.bd.getActiveCharacter().modifyAP(BattleGUI.ITEM_ACTION_POINTS);
		}
		this.updateStats();
		final int currResult = this.getResult();
		if (currResult != BattleResults.IN_PROGRESS) {
		...
		// Active character has AI
		final CombatUsableItem cui = this.bd.getActiveCharacter().getTemplate().getAI().getItemToUse();
		final boolean success = CombatItemManager.useItem(cui, this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.bd.getActiveCharacter().modifyAP(BattleGUI.ITEM_ACTION_POINTS);
		}
	...
	if (this.getActiveActionCounter() > 0) {
	    ...
	    int stealChance;
	    int stealAmount = 0;
	    this.bd.getActiveCharacter().modifyAP(BattleGUI.STEAL_ACTION_POINTS);
	    this.updateStats();
	    try {
		final Caste caste = ((PartyMember) this.bd.getActiveCharacter().getTemplate()).getCaste();
	...
	if (this.getActiveActionCounter() > 0) {
	    ...
	    int drainChance;
	    int drainAmount = 0;
	    this.bd.getActiveCharacter().modifyAP(BattleGUI.DRAIN_ACTION_POINTS);
	    this.updateStats();
	    try {
		final Caste caste = ((PartyMember) this.bd.getActiveCharacter().getTemplate()).getCaste();
		...
		// Succeeded
		final RandomRange drained = new RandomRange(0, activeEnemy.getGold());
	...
	if (this.battleFrame.isVisible()) {
		...
		for (y = yView; y <= ylView; y++) {
		    ...
		    yFix = y - yView;
		    try {
			final BufferedImageIcon icon1 = this.bd.getBattleMap().getBattleGround(y, x).battleRenderHook(x,
				y);
			final BufferedImageIcon icon2 = this.bd.getBattleMap().getBattleCell(y, x).battleRenderHook(x,
				y);
			this.drawGrid[xFix][yFix].setIcon(ImageTransformer.getCompositeImage(icon1, icon2));
		    } catch (final ArrayIndexOutOfBoundsException ae) {
			this.drawGrid[xFix][yFix].setIcon(MapObjectImageManager.getImage("Void", "Void", null));
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (!this.bd.getBattlers()[x].getTemplate().isAlive()) {
		    ...
		    // Remove character from battle
		    this.bd.getBattleMap().setBattleCell(new Empty(), this.bd.getBattlers()[x].getX(),
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlerAIContexts()[x] != null) {
		    this.bd.getBattlerAIContexts()[x].updateContext(this.bd.getBattleMap());
		}
	    }
	}
    }
	...
	for (int x = 0; x < this.vwMgr.getViewingWindowSizeX(); x++) {
		...
		this.drawGrid[x][y] = new JLabel("", null, SwingConstants.CENTER);
