...
public class GameManager {
	...
	return DungeonDiver2.getApplication().getVariablesManager().getMap().getTemporaryBattleCopy();
    }
	...
	final GenericCharacter player = (GenericCharacter) DungeonDiver2.getApplication().getVariablesManager().getMap()
	...
	final Map m = DungeonDiver2.getApplication().getVariablesManager().getMap();
	...
	do {
		...
		try {
		    nextBelow = m.getCell(px + x, py + y, pz, MapConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextBelow = new Empty();
		}
		try {
		    nextAbove = m.getCell(px + x, py + y, pz, MapConstants.LAYER_OBJECT);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextAbove = new Wall();
		}
		try {
		...
		try {
		    if (this.checkSolid(this.savedMapObject, below, nextBelow, nextAbove)) {
			m.setCell(this.savedMapObject, px, py, pz, MapConstants.LAYER_OBJECT);
			...
			this.savedMapObject = m.getCell(px, py, pz, MapConstants.LAYER_OBJECT);
			m.setCell(new Player(), px, py, pz, MapConstants.LAYER_OBJECT);
			app.getVariablesManager().setDirty(true);
			this.redrawMap();
			groundInto = m.getCell(px, py, pz, MapConstants.LAYER_GROUND);
			if (groundInto.overridesDefaultPostMove()) {
			    GameScriptRunner.runScript(groundInto.getPostMoveScript(false, px, py, pz, m));
			    if (!this.savedMapObject.isOfType(TypeConstants.TYPE_PASS_THROUGH)) {
				GameScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, px, py, pz, m));
			    }
			} else {
			    GameScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, px, py, pz, m));
			}
			...
			this.fireMoveFailedActions(px + x, py + y, this.savedMapObject, below, nextBelow, nextAbove);
			proceed = false;
		    }
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    ...
		    DungeonDiver2.getApplication().showMessage("Can't go that way");
		    o = new Empty();
		    proceed = false;
		}
	    } else {
		// Move failed - pre-move check failed
		GameScriptRunner.runScript(o.getMoveFailedScript(false, px + x, py + y, pz));
		proceed = false;
	    }
	    if (redrawsSuspended && !this.checkLoopCondition(proceed, groundInto, below, nextBelow, nextAbove)) {
		// Redraw post-suspend
		this.redrawMap();
		redrawsSuspended = false;
	    }
	} while (this.checkLoopCondition(proceed, groundInto, below, nextBelow, nextAbove));
	// Post-move actions
	final ArrayList<GameScriptArea> areaScripts = app.getVariablesManager().getMap()
		.getScriptAreasAtPoint(new Point(x, y));
    ...
    private boolean checkLoopCondition(final boolean proceed, final MapObject groundInto, final MapObject below,
	    final MapObject nextBelow, final MapObject nextAbove) {
	return proceed && !groundInto.hasFrictionConditionally(false)
		&& this.checkSolid(this.savedMapObject, below, nextBelow, nextAbove);
    }

    private boolean checkSolid(final MapObject inside, final MapObject below, final MapObject nextBelow,
    ...
    private void fireMoveFailedActions(final int x, final int y, final MapObject inside, final MapObject below,
	    final MapObject nextBelow, final MapObject nextAbove) {
	final Map m = DungeonDiver2.getApplication().getVariablesManager().getMap();
	final int z = this.plMgr.getPlayerLocationZ();
	final boolean insideSolid = inside.isConditionallySolid(m, z);
	...
	final boolean nextAboveSolid = nextAbove.isConditionallySolid(m, z);
	if (insideSolid) {
	    GameScriptRunner.runScript(inside.getMoveFailedScript(false, x, y, z));
	}
	if (belowSolid) {
	    GameScriptRunner.runScript(below.getMoveFailedScript(false, x, y, z));
	}
	if (nextBelowSolid) {
	    GameScriptRunner.runScript(nextBelow.getMoveFailedScript(false, x, y, z));
	}
	if (nextAboveSolid) {
	    GameScriptRunner.runScript(nextAbove.getMoveFailedScript(false, x, y, z));
	}
    }
	...
	final Map m = app.getVariablesManager().getMap();
	try {
	    m.getCell(x, y, z, MapConstants.LAYER_OBJECT).preMoveCheck(true, x, y, z, m);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	} catch (final NullPointerException np) {
	    // Do nothing
	...
	this.vwMgr.saveViewingWindow();
	try {
	    if (!m.getCell(x, y, z, MapConstants.LAYER_OBJECT).isConditionallySolid(m, z)) {
		m.setCell(this.savedMapObject, this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
		...
		this.savedMapObject = m.getCell(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
		m.setCell(new Player(), this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
		this.redrawMap();
		app.getVariablesManager().setDirty(true);
		GameScriptRunner.runScript(this.savedMapObject.getPostMoveScript(false, x, y, z, m));
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    this.plMgr.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    m.setCell(new Player(), this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
		    this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
	    DungeonDiver2.getApplication().showMessage("Can't go outside the map");
	} catch (final NullPointerException np) {
	    this.plMgr.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    m.setCell(new Player(), this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
	...
	try {
		...
		m.setCell(this.savedMapObject, this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
		this.plMgr.setPlayerLocation(x, y, z, this.plMgr.getPlayerLocationW());
		this.vwMgr.setViewingWindowLocationX(this.plMgr.getPlayerLocationY() - this.vwMgr.getOffsetFactorX());
		this.vwMgr.setViewingWindowLocationY(this.plMgr.getPlayerLocationX() - this.vwMgr.getOffsetFactorY());
		this.savedMapObject = m.getCell(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MapConstants.LAYER_OBJECT);
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
		    ...
		    visible = app.getVariablesManager().getMap().isSquareVisible(u, v, y, x,
			    this.plMgr.getPlayerLocationZ());
		    try {
			if (visible) {
			    MapObject obj1, obj2;
			    obj1 = app.getVariablesManager().getMap().getCell(y, x, this.plMgr.getPlayerLocationZ(),
				    MapConstants.LAYER_GROUND);
			    obj2 = app.getVariablesManager().getMap().getCell(y, x, this.plMgr.getPlayerLocationZ(),
				    MapConstants.LAYER_OBJECT);
			    String name1, name2;
			    name1 = obj1.gameRenderHook(y, x, this.plMgr.getPlayerLocationZ(),
				    app.getVariablesManager().getMap());
			    name2 = obj2.gameRenderHook(y, x, this.plMgr.getPlayerLocationZ(),
				    app.getVariablesManager().getMap());
			    this.drawGrid[xFix][yFix].setIcon(ImageTransformer.getCompositeImage(name1,
				    obj1.getGameName(), obj1.getTemplateTransform(), name2, obj2.getGameName(),
				    obj2.getTemplateTransform()));
			} else {
			    this.drawGrid[xFix][yFix]
				    .setIcon(MapObjectImageManager.getImage("Darkness", "Darkness", null));
			}
		    } catch (final ArrayIndexOutOfBoundsException ae) {
			this.drawGrid[xFix][yFix].setIcon(MapObjectImageManager
				.getImage(new EmptyVoid().gameRenderHook(y, x, this.plMgr.getPlayerLocationZ(),
					app.getVariablesManager().getMap()), "Void", null));
		    } catch (final NullPointerException np) {
			this.drawGrid[xFix][yFix].setIcon(MapObjectImageManager
				.getImage(new EmptyVoid().gameRenderHook(y, x, this.plMgr.getPlayerLocationZ(),
					app.getVariablesManager().getMap()), "Void", null));
	...
	final Application app = DungeonDiver2.getApplication();
	final Map m = app.getVariablesManager().getMap();
	final boolean floorExists = m.doesFloorExist(this.plMgr.getPlayerLocationZ() + floor);
	if (floorExists) {
	    final int px = this.plMgr.getPlayerLocationX();
	    final int py = this.plMgr.getPlayerLocationY();
	    int pz = this.plMgr.getPlayerLocationZ();
	...
	if (member != null) {
	    ...
	    CommonDialogs.showInputDialog("Equipment", "Equipment", equipString, equipString[0]);
	}
    }
	...
	if (member != null) {
	    ...
	    CommonDialogs.showInputDialog("Item Inventory", "Item Inventory", invString, invString[0]);
	}
    }
	...
	final int destX = x / MapObjectImageManager.getGraphicSize() + this.vwMgr.getViewingWindowLocationX() - xOffset
		+ yOffset;
	final int destY = y / MapObjectImageManager.getGraphicSize() + this.vwMgr.getViewingWindowLocationY() + xOffset
		- yOffset;
	final int destZ = this.plMgr.getPlayerLocationZ();
	try {
	    final MapObject target1 = m.getCell(destX, destY, destZ, MapConstants.LAYER_GROUND);
	    final MapObject target2 = m.getCell(destX, destY, destZ, MapConstants.LAYER_OBJECT);
	    ...
	    DungeonDiver2.getApplication().showMessage(gameName2 + " on " + gameName1);
