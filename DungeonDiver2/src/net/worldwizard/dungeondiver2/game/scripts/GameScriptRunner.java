...
public final class GameScriptRunner {
	...
	int actionCounter = 0;
	try {
	    final GameManager gm = DungeonDiver2.getApplication().getGameManager();
	    if (s != null) {
		...
		for (int x = 0; x < totAct; x++) {
		    ...
		    case SHOP:
			// Show the shop
			final int shopType = se.getFirstActionArg().getInteger();
			final Shop shop = DungeonDiver2.getApplication().getGenericShop(shopType);
			if (shop != null) {
			    shop.showShop();
			} else {
			    throw new IllegalArgumentException("Illegal Shop Type: " + shopType);
			}
			break;
		    case MOVE:
			// Move
			final boolean moveType = se.getActionArg(0).getBoolean();
			final boolean eventFlag = se.getActionArg(1).getBoolean();
			...
			if (moveType == GameScriptConstants.MOVE_RELATIVE) {
				...
				gm.updatePositionAbsoluteNoEvents(moveX, moveY, moveZ);
			    }
			}
			break;
		    case END_GAME:
		    ...
		    case MODIFY:
			// Modify
			final boolean stickyScript = se.getActionArg(0).getBoolean();
			final String modObjName = se.getActionArg(1).getString();
			...
			final MapObject modObj = DungeonDiver2.getApplication().getObjects()
				.getNewInstanceByName(modObjName);
			if (stickyScript) {
			    final MapObject wasThere = DungeonDiver2.getApplication().getVariablesManager().getMap()
				    .getCell(modX, modY, modZ, modL);
			    if (wasThere.hasCustomScript()) {
				modObj.setCustomScript(wasThere.getCustomScript());
			    }
			}
			DungeonDiver2.getApplication().getVariablesManager().getMap().setCell(modObj, modX, modY, modZ,
				modL);
			break;
		    case DELETE_SCRIPT:
			// Delete Script
		    ...
		    case BATTLE:
			// Hide the game
			DungeonDiver2.getApplication().getGameManager().hideOutput();
			// Battle
			...
			new Thread("Battle") {
			    ...
			    public void run() {
				try {
				    DungeonDiver2.getApplication().getBattle().doFixedBattle(
					    DungeonDiver2.getApplication().getGameManager().getTemporaryBattleCopy(),
					    battle);
			...
			DungeonDiver2.getApplication().getGameManager().goToLevel(destLevel);
			break;
		    case RELATIVE_LEVEL_CHANGE:
			final int rDestLevel = se.getActionArg(0).getInteger();
			DungeonDiver2.getApplication().getGameManager().goToLevelRelative(rDestLevel);
			break;
		    case ADD_TO_SCORE:
			final int points = se.getActionArg(0).getInteger();
			DungeonDiver2.getApplication().getGameManager().addToScore(points);
			break;
		    case SWAP_PAIRS:
			final String swap1 = se.getActionArg(0).getString();
			final String swap2 = se.getActionArg(1).getString();
			final MapObject swapObj1 = DungeonDiver2.getApplication().getObjects()
				.getNewInstanceByName(swap1);
			final MapObject swapObj2 = DungeonDiver2.getApplication().getObjects()
			...
			break;
		    case REDRAW:
			DungeonDiver2.getApplication().getGameManager().redrawMapNoRebuild();
			break;
		    default:
			throw new IllegalArgumentException("Illegal Action Code: " + code.toString());
		    }
		}
	    }
	} catch (final Exception e) {
	    final int px = DungeonDiver2.getApplication().getGameManager().getPlayerManager().getPlayerLocationX() + 1;
	    final int py = DungeonDiver2.getApplication().getGameManager().getPlayerManager().getPlayerLocationY() + 1;
	    final int pz = DungeonDiver2.getApplication().getGameManager().getPlayerManager().getPlayerLocationZ() + 1;
	    final int pw = DungeonDiver2.getApplication().getGameManager().getPlayerManager().getPlayerLocationW() + 1;
	    final String beginMsg = "Buggy Game Script at row " + px + ", column " + py + ", floor " + pz + ", level "
	...
	final GameActionCode code = se.getActionCode();
	final int rargc = GameScriptConstants.ARGUMENT_COUNT_VALIDATION[code.ordinal()];
	int aargc;
	try {
	...
	if (rargc != aargc) {
	    throw new IllegalArgumentException("Expected " + rargc + " arguments, found " + aargc + " arguments!");
	}
	final Class<?>[] rargt = GameScriptConstants.ARGUMENT_TYPE_VALIDATION[code.ordinal()];
	if (rargt != null) {
		...
		if (!aargt[x].getName().equals(rargt[x].getName())) {
		    throw new IllegalArgumentException("Expected argument of type " + rargt[x].getName()
			    + " at position " + (x + 1) + ", found " + aargt[x].getName() + "!");
		}
	    }
	}
    }
