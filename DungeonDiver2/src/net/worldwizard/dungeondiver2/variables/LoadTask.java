...
public class LoadTask extends Thread {
	...
	try {
		...
		ZipUtilities.unzipDirectory(variablesFile, new File(Support.getVariables().getBasePath()));
		// Load variables data
		...
		if (this.gameMap.doMapsExist()) {
		    ...
		    if (playerExists) {
			app.getGameManager().getPlayerManager().setPlayerLocation(this.gameMap.getStartColumn(),
				this.gameMap.getStartRow(), this.gameMap.getStartFloor(), startW);
		...
		// Final cleanup
		final String lum = app.getVariablesManager().getLastUsedVariables();
		...
		throw new InvalidMapException("Error loading " + sg.toLowerCase() + " file.");
	    }
