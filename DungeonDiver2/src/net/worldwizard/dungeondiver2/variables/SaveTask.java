...
public class SaveTask extends Thread {
	...
	try {
	    ...
	    // Set prefix handler
	    app.getVariablesManager().getMap().setXPrefixHandler(new PrefixHandler());
	    // Set suffix handler
	    app.getVariablesManager().getMap().setXSuffixHandler(new SuffixHandler());
	    // Save start location
	    app.getVariablesManager().getMap().saveStart();
	    // Save active level
	    this.savedLevel = app.getVariablesManager().getMap().getActiveLevelNumber();
	    ...
	    ZipUtilities.zipDirectory(new File(Support.getVariables().getBasePath()), mapFile);
