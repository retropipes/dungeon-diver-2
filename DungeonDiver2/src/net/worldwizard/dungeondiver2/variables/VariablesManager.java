...
public class VariablesManager implements OpenFilesHandler {
	...
	final FileDialog fc = new FileDialog(app.getOutputFrame(), "Load Saved Game", FileDialog.LOAD);
	...
	if (!FilenameChecker
		.isFilenameOK(VariablesManager.getNameWithoutExtension(VariablesManager.getFileNameOnly(filename)))) {
	    CommonDialogs.showErrorDialog("The file you selected contains illegal characters in its\n"
		    ...
		    + "named com1 through com9 and lpt1 through lpt9.", "Load Variables or Saved Game");
	} else {
	...
	if (app.getMode() == Application.STATUS_GAME) {
		...
		final String extension = VariablesManager.getExtension(this.lastUsedGameFile);
		if (extension != null) {
		    if (!extension.equals(Extension.getGameExtension())) {
			this.lastUsedGameFile = VariablesManager.getNameWithoutExtension(this.lastUsedGameFile)
	...
	final FileDialog fc = new FileDialog(app.getOutputFrame(), "Save Game", FileDialog.SAVE);
	...
	while (!FilenameChecker.isFilenameOK(fileOnly)) {
		...
		if (!FilenameChecker.isFilenameOK(fileOnly)) {
			...
			if (extension != null) {
			    if (!extension.equals(Extension.getGameExtension())) {
				filename = VariablesManager.getNameWithoutExtension(file)
					+ Extension.getGameExtensionWithPeriod();
			    }
			} else {
