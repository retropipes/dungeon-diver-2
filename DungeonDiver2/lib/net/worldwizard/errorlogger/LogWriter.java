...
class LogWriter {
    ...
    // Constructors
    public LogWriter(final Throwable problem, final String programName) {
	this.t = problem;
	...
	try {
	    ...
	    // Print to the file
	    try (final PrintStream s = new PrintStream(new BufferedOutputStream(new FileOutputStream(df)))) {
	...
	final SimpleDateFormat sdf = new SimpleDateFormat("'_'yyyyMMdd'_'HHmmssSSS");
