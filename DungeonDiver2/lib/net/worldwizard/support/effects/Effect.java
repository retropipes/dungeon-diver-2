...
public class Effect extends Identifiable implements StatConstants {
    ...
    public Effect(final String effectName, final int newMinRounds, final int newMaxRounds) {
	super(true);
	this.name = effectName;
	this.messages = new String[Effect.MAX_MESSAGES];
	this.effect = new double[Effect.MAX_EFFECT_TYPES][StatConstants.MAX_STATS];
    ...
    public void setEffect(final int type, final int stat, final double value, final double factor,
	    final int scaleStat) {
    ...
    public void modifyEffect(final int type, final int stat, final double value, final double factor,
	    final int scaleStat) {
	this.effect[type][stat] = value;
	this.effectScaleFactor = factor;
	this.effectScaleStat = scaleStat;
	...
	longHash = longHash
		.add(IDGenerator.computeStringLongHash(this.getClass().getName()).multiply(BigInteger.valueOf(2)));
	longHash = longHash.add(IDGenerator.computeStringLongHash(this.name).multiply(BigInteger.valueOf(3)));
	longHash = longHash
		.add(IDGenerator.computeDoubleLongHash(this.effectScaleFactor).multiply(BigInteger.valueOf(4)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.effectScaleStat).multiply(BigInteger.valueOf(5)));
	longHash = longHash
		.add(IDGenerator.computeDoubleLongHash(this.roundsScaleFactor).multiply(BigInteger.valueOf(6)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.roundsScaleStat).multiply(BigInteger.valueOf(7)));
	for (int x = 0; x < Effect.MAX_MESSAGES; x++) {
	    longHash = longHash
		    .add(IDGenerator.computeStringLongHash(this.messages[x]).multiply(BigInteger.valueOf(x + 8)));
	}
	return longHash;
    }
	...
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "effects");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
