...
public class StealEffect extends Effect {
    // Constructor
    public StealEffect(final String buffName, final int GPAddition, final int newRounds, final double factor,
	    final int scaleStat, final double decay) {
	super(buffName, newRounds);
	this.setEffect(Effect.EFFECT_ADD, StatConstants.STAT_GOLD, GPAddition, factor, scaleStat);
	...
	if (target.isAlive()) {
	    target.offsetGold((int) this.getEffect(Effect.EFFECT_ADD, StatConstants.STAT_GOLD));
	}
    }
}
