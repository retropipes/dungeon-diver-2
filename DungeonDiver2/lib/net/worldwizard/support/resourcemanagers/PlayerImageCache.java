...
public class PlayerImageCache {
    ...
    // Methods
    static BufferedImageIcon getCachedPlayerImage(final String name, final Element element) {
	final String cacheName = element.getName() + " " + name;
	if (!PlayerImageCache.isInCache(cacheName)) {
	    final BufferedImageIcon bii = PlayerImageManager.getUncachedPlayerImage(name, element);
	...
	final String[] tempNameCache = new String[PlayerImageCache.cache.length + PlayerImageCache.CACHE_INCREMENT];
	...
	if (PlayerImageCache.cache == null || PlayerImageCache.nameCache == null) {
