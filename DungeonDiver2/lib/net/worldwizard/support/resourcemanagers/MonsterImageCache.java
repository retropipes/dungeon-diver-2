...
public class MonsterImageCache {
    ...
    // Methods
    static BufferedImageIcon getCachedMonsterImage(final String name, final Element element) {
	final String cacheName = element.getName() + " " + name;
	if (!MonsterImageCache.isInCache(cacheName)) {
	    final BufferedImageIcon bii = MonsterImageManager.getUncachedMonsterImage(name, element);
	...
	final String[] tempNameCache = new String[MonsterImageCache.cache.length + MonsterImageCache.CACHE_INCREMENT];
	...
	if (MonsterImageCache.cache == null || MonsterImageCache.nameCache == null) {
