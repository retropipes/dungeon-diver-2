...
public class MapObjectImageCache {
    ...
    static BufferedImageIcon getCachedImage(final String name, final String rawName, final TemplateTransform tt) {
	String cacheName;
	if (tt == null) {
	    cacheName = rawName;
	} else {
	...
	if (!MapObjectImageCache.isInCache(cacheName)) {
	    final BufferedImageIcon bii = MapObjectImageManager.getUncachedImage(name, tt);
	...
	if (MapObjectImageCache.cache == null || MapObjectImageCache.nameCache == null) {
