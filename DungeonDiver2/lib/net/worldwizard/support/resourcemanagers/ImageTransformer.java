...
public class ImageTransformer {
    ...
    static BufferedImageIcon getTemplateTransformedImage(final BufferedImageIcon icon, final TemplateTransform tt) {
	try {
	    if (tt == null) {
		return icon;
	    } else {
    ...
    public static BufferedImageIcon getTransformedImage(final String name, final String raw,
	    final TemplateTransform tt) {
	try {
	    final BufferedImageIcon icon = MapObjectImageCache.getCachedImage(name, raw, tt);
	    if (icon != null) {
		final BufferedImageIcon result = new BufferedImageIcon(icon);
		for (int x = 0; x < MapObjectImageManager.getGraphicSize(); x++) {
		    for (int y = 0; y < MapObjectImageManager.getGraphicSize(); y++) {
    ...
    public static BufferedImageIcon getCompositeImage(final String name1, final String raw1,
	    final TemplateTransform tt1, final String name2, final String raw2, final TemplateTransform tt2) {
	try {
	    final BufferedImageIcon icon1 = MapObjectImageCache.getCachedImage(name1, raw1, tt1);
	    final BufferedImageIcon icon2 = MapObjectImageCache.getCachedImage(name2, raw2, tt2);
