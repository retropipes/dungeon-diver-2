...
public class Battle extends Identifiable {
	...
	for (int x = 0; x < numMonsters; x++) {
	    this.monsterArray[x] = MonsterManager.getNewMonsterInstance(true, true, false);
	}
    }
	...
	for (int x = 0; x < Battle.MAX_MONSTERS; x++) {
	    longHash = longHash.add(this.monsterArray[x].computeLongHash()).multiply(BigInteger.valueOf(x + 3));
	}
	return longHash;
    }
	...
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "battles");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
	try (final XDataWriter writer = DataIOFactory
		.createTagWriter(
			Support.getSystemVariables().getBasePath() + File.separator + "battles" + File.separator
