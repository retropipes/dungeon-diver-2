...
public class SpellCaster {
    ...
    public static boolean selectAndCastSpell(final Creature caster, final int teamID, final boolean aiEnabled,
	...
	if (s != null) {
	    ...
	    if (power != -1) {
		result = SpellCaster.castSpellWithPower(s, power, caster, teamID, aiEnabled, battle);
	...
	final String[] powers = new String[] { "1", "2", "3", "4", "5", "6", "7", "8", "9", "X" };
	String dialogResult = null;
	dialogResult = CommonDialogs.showInputDialog("Select a Spell Power Level", "Select Power", powers, powers[0]);
	if (dialogResult != null) {
	    int index;
    ...
    public static boolean castSpell(final Spell cast, final Creature caster, final int teamID, final boolean aiEnabled,
	    final BattleDefinitions battle) {
	if (cast != null) {
		...
		eff.resetEffect();
		final Creature[] targets = SpellCaster.resolveTarget(cast, caster, teamID, aiEnabled, battle);
    ...
    public static boolean castSpellWithPower(final Spell cast, final int power, final Creature caster, final int teamID,
    ...
    private static Creature[] resolveTarget(final Spell cast, final Creature caster, final int teamID,
	    final boolean aiEnabled, final BattleDefinitions battle) {
	...
	case ONE_ALLY:
	    // One Ally
	    if (useAI) {
		return new Creature[] { battle.pickOneFriendOfTeamRandomly(teamID) };
	    } else {
	...
	case ONE_ENEMY:
	    // One Enemy
	    if (useAI) {
		return new Creature[] { battle.pickOneEnemyOfTeamRandomly(teamID) };
	    } else {
