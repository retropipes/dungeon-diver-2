...
public class Spell extends Identifiable {
    ...
    public Spell(final Effect newEffect, final int newCost, final BattleTarget newTarget, final GameSounds sfx) {
	super(true);
	...
	longHash = longHash.add(this.effect.computeLongHash().multiply(BigInteger.valueOf(2)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.cost).multiply(BigInteger.valueOf(3)));
	longHash = longHash
		.add(IDGenerator.computeLongLongHash(this.soundEffect.hashCode()).multiply(BigInteger.valueOf(4)));
	return longHash;
    }
	...
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "spells");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
