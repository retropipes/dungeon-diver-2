...
class LayeredTower implements Cloneable {
	...
	this.data = new LowLevelDataStore(cols, rows, floors, MapConstants.LAYER_COUNT);
    ...
    public MapObject getCell(final int row, final int col, final int floor, final int extra) {
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    final MapObject mo = this.getCell(y, x, z, MapConstants.LAYER_OBJECT);
		    if (mo != null) {
    ...
    public boolean isSquareVisible(final int x1, final int y1, final int x2, final int y2, final int zLoc) {
	...
	if (xDist <= this.visionRadius && yDist <= this.visionRadius) {
	    if (this.radialScan(x2, y2, zLoc, MapConstants.LAYER_OBJECT, dg.getEffectRadius(), dg.getName())) {
		result = false;
	    } else {
		result = true;
	    }
	} else {
	    if (this.radialScan(x2, y2, zLoc, MapConstants.LAYER_OBJECT, lg.getEffectRadius(), lg.getName())) {
		result = true;
	    } else {
		result = false;
	    }
    ...
    public void setCell(final MapObject mo, final int row, final int col, final int floor, final int extra) {
    ...
    public void fillFloor(final MapObject bottom, final MapObject top, final int z) {
	int x, y, e;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
    ...
    public void fillRandomly(final Map map, final int w, final MapObject pass1FillBottom) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorRandomly(map, z, w, pass1FillBottom);
	}
    }

    public void fillFloorRandomly(final Map map, final int z, final int w, final MapObject pass1FillBottom) {
	// Pre-Pass
	...
	for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
	    final MapObject[] objectsWithoutPrerequisites = objects.getAllNotRequired(e);
		...
		if (e == MapConstants.LAYER_GROUND) {
			...
			for (y = 0; y < rows; y += this.regionSize) {
			    ...
			    final boolean okay = placeObj.shouldGenerateObject(map, x, y, z, w, e);
			    if (okay) {
				for (u = 0; u < this.regionSize; u++) {
				    for (v = 0; v < this.regionSize; v++) {
					this.setCell(objects.getNewInstanceByName(placeObj.getName()), v + x, u + y, z,
						e);
				    }
				}
			    }
	...
	for (int layer = 0; layer < MapConstants.LAYER_COUNT; layer++) {
		...
		for (x = 0; x < requiredObjects.length; x++) {
			...
			randomRow = row.generate();
			randomColumn = column.generate();
			if (currObj.shouldGenerateObject(map, randomRow, randomColumn, z, w, layer)) {
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
				    layer);
			} else {
			    while (!currObj.shouldGenerateObject(map, randomColumn, randomRow, z, w, layer)) {
				randomRow = row.generate();
				randomColumn = column.generate();
			    }
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
	...
	for (final GameScriptArea gsa : this.scriptAreas) {
	    if (p.x >= gsa.getUpperLeft().x && p.x <= gsa.getLowerRight().x && p.y >= gsa.getUpperLeft().y
    ...
    public static LayeredTower readXLayeredTowerV1(final XDataReader reader, final int ver) throws IOException {
	final MapObjectList objects = new MapObjectList();
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
			lt.setCell(objects.readMapObjectX(reader, ver), y, x, z, e);
    ...
    public void writeSavedTowerStateX(final XDataWriter writer) throws IOException {
	this.savedTowerState.writeSavedTowerStateX(writer);
    }

    public void readSavedTowerStateX(final XDataReader reader, final int formatVersion) throws IOException {
	this.savedTowerState = SavedTowerState.readSavedTowerStateX(reader, formatVersion);
    }
}
