...
class SavedTowerState implements Cloneable {
    ...
    public MapObject getDataCell(final int x, final int y, final int z, final int e) {
	return this.saveData[x][y][z][e];
    }

    public void setDataCell(final MapObject newData, final int x, final int y, final int z, final int e) {
    ...
    public static SavedTowerState readSavedTowerStateX(final XDataReader reader, final int formatVersion)
	    throws IOException {
	...
	for (x = 0; x < sts.c; x++) {
		...
		for (z = 0; z < sts.f; z++) {
		    for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
			sts.saveData[x][y][z][e] = objects.readMapObjectX(reader, formatVersion);
		    }
		}
	    }
	}
