...
public class Springboard extends GenericTeleport {
	...
	entry2.addActionArg(new GameScriptEntryArgument(GameSounds.JUMPING));
    ...
    private boolean searchNestedSprings(final int dirX, final int dirY, final int floor, final Map map) {
	...
	if (!map.doesFloorExist(floor + 1)) {
	    ...
	    return super.shouldGenerateObject(map, row, col, floor, level, layer);
	}
    }
