...
public class EmptyVoid extends GenericWall {
    ...
    @Override
    public String gameRenderHook(final int x, final int y, final int z, final Map map) {
    ...
    @Override
    public void determineCurrentAppearance(final int x, final int y, final int z, final Map map) {
	String mo1Name, mo2Name, mo3Name, mo4Name, mo6Name, mo7Name, mo8Name, mo9Name, thisName;
	thisName = this.getName();
	final MapObject mo1 = map.getCell(x - 1, y - 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo1Name = mo1.getName();
	} catch (final NullPointerException np) {
	    mo1Name = thisName;
	}
	final MapObject mo2 = map.getCell(x - 1, y, z, MapConstants.LAYER_OBJECT);
	try {
	    mo2Name = mo2.getName();
	} catch (final NullPointerException np) {
	    mo2Name = thisName;
	}
	final MapObject mo3 = map.getCell(x - 1, y + 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo3Name = mo3.getName();
	} catch (final NullPointerException np) {
	    mo3Name = thisName;
	}
	final MapObject mo4 = map.getCell(x, y - 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo4Name = mo4.getName();
	} catch (final NullPointerException np) {
	    mo4Name = thisName;
	}
	final MapObject mo6 = map.getCell(x, y + 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo6Name = mo6.getName();
	} catch (final NullPointerException np) {
	    mo6Name = thisName;
	}
	final MapObject mo7 = map.getCell(x + 1, y - 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo7Name = mo7.getName();
	} catch (final NullPointerException np) {
	    mo7Name = thisName;
	}
	final MapObject mo8 = map.getCell(x + 1, y, z, MapConstants.LAYER_OBJECT);
	try {
	    mo8Name = mo8.getName();
	} catch (final NullPointerException np) {
	    mo8Name = thisName;
	}
	final MapObject mo9 = map.getCell(x + 1, y + 1, z, MapConstants.LAYER_OBJECT);
	try {
	    mo9Name = mo9.getName();
	} catch (final NullPointerException np) {
	    mo9Name = thisName;
