...
public class Map implements MapConstants {
	...
	this.mapBasePath = Support.getVariables().getBasePath() + File.separator + "maps" + File.separator;
	...
	final RandomRange gen = new RandomRange(0, list.getAllGroundLayerObjects().length - 1);
    ...
    private void readMapLevelX(final XDataReader reader, final int formatVersion) throws IOException {
	if (formatVersion == FormatConstants.SCENARIO_FORMAT_1) {
	    this.mapData = LayeredTower.readXLayeredTowerV1(reader, formatVersion);
