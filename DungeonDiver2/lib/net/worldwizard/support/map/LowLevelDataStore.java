...
class LowLevelDataStore implements Cloneable {
    // Fields
    private final MapObject[] dataStore;
	...
	this.dataStore = new MapObject[product];
    }
    ...
    public MapObject getCell(final int... loc) {
	final int aloc = this.ravelLocation(loc);
	return this.dataStore[aloc];
    }

    public void setCell(final MapObject obj, final int... loc) {
	final int aloc = this.ravelLocation(loc);
