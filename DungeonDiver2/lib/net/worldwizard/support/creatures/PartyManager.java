...
public class PartyManager {
	...
	PartyManager.party = new Party(PartyManager.PARTY_SIZE, PartyManager.PARTY_PLAYER_LIMIT);
	int mem = 0;
	...
	for (int x = 0; x < PartyManager.PARTY_PLAYER_LIMIT; x++) {
	    ...
	    mem++;
	    final int response = CommonDialogs.showConfirmDialog("Add another character?", "Create Party");
    ...
    public static PartyMember getNewPCInstance(final int r, final int c, final int f, final int g, final int p,
	    final String n) {
	...
	if (name != null) {
		...
		if (caste != null) {
			...
			if (gender != null) {
			    ...
			    if (personality != null) {
				return new PartyMember(race, caste, faith, gender, personality, name);
			    }
			}
		    }
		}
