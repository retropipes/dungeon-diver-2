...
public abstract class Creature extends Identifiable implements StatConstants {
	...
	return (int) Math.ceil(
		this.getEffectedStat(StatConstants.STAT_AGILITY) * StatConstants.FACTOR_AGILITY_ACTIONS_PER_ROUND);
    }
	...
	return "Attack: " + this.getEffectedStat(StatConstants.STAT_ATTACK) + " (" + this.getAttack() + ")";
    }
	...
	return "Defense: " + this.getEffectedStat(StatConstants.STAT_DEFENSE) + " (" + this.getDefense() + ")";
    }
	...
	return (int) (this.getEffectedStat(StatConstants.STAT_AGILITY) * StatConstants.FACTOR_AGILITY_SPEED
		- this.items.getTotalEquipmentWeight() * StatConstants.FACTOR_WEIGHT_SPEED);
    }
	...
	for (x = 0; x < this.effectList.length; x++) {
	    try {
		final Effect e = this.get(x);
		p *= e.getEffect(Effect.EFFECT_MULTIPLY, StatConstants.STAT_ATTACK);
	    } catch (final NullPointerException np) {
		// Do nothing
	    } catch (final ArrayIndexOutOfBoundsException aioob) {
		// Do nothing
	...
	if (this.dynamism[stat] != 0) {
	    final RandomRange r = new RandomRange(-this.dynamism[stat], this.dynamism[stat]);
	...
	longHash = longHash.add(IDGenerator.computeStringLongHash(this.getName()).multiply(BigInteger.valueOf(2)));
	return longHash;
    }
}
