...
public abstract class BaseMonster extends Creature {
	...
	longHash = longHash.add(IDGenerator.computeStringLongHash(this.type).multiply(BigInteger.valueOf(2)));
	return longHash;
    }

    @Override
    public void dumpContentsToFile() throws IOException {
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "monsters");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
