...
public abstract class DefiniteScalingBaseMonster extends DefiniteBaseMonster {
    // Constructors
	...
	final Equipment helmet = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_HEAD);
	final Equipment necklace = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_NECK);
	final Equipment shield = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_OFFHAND);
	final Equipment robe = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_BODY);
	final Equipment cape = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_BACK);
	final Equipment shirt = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_UPPER_TORSO);
	final Equipment bracers = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_ARMS);
	final Equipment gloves = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_HANDS);
	final Equipment ring = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_FINGERS);
	final Equipment belt = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_LOWER_TORSO);
	final Equipment pants = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_LEGS);
	final Equipment boots = EquipmentFactory.createArmor(newLevel, EquipmentSlotConstants.SLOT_FEET);
	// Equip it
	...
	final RandomRange r = new RandomRange(1, this.getLevel() * StatConstants.GAIN_STRENGTH);
	return r.generate();
    }

    private int getInitialBlock() {
	final RandomRange r = new RandomRange(0, this.getLevel() * StatConstants.GAIN_BLOCK);
	return r.generate();
    }
	...
	final int min = 0;
	final int max = this.getToughness() * BaseMonster.GOLD_TOUGHNESS_MULTIPLIER;
	final RandomRange r = new RandomRange(min, max);
	return r.generate();
    }

    private int getInitialAgility() {
	final RandomRange r = new RandomRange(1, this.getLevel() * StatConstants.GAIN_AGILITY);
	return r.generate();
    }

    private int getInitialVitality() {
	final RandomRange r = new RandomRange(1, this.getLevel() * StatConstants.GAIN_VITALITY);
	return r.generate();
    }

    private int getInitialIntelligence() {
	final RandomRange r = new RandomRange(0, this.getLevel() * StatConstants.GAIN_INTELLIGENCE);
	return r.generate();
    }

    private int getInitialLuck() {
	final RandomRange r = new RandomRange(0, this.getLevel() * StatConstants.GAIN_LUCK);
	return r.generate();
    }
}
