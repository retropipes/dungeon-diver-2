...
public class PartyMember extends Creature {
    ...
    // Constructors
    public PartyMember(final Race r, final Caste c, final Faith f, final Gender g, final Personality p,
	    final String n) {
	super();
	this.name = n;
	this.race = r;
	...
	this.personality = p;
	this.setLevel(1);
	final int STR = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_STRENGTH);
	this.rSTR = new RandomRange(-STR, STR);
	final int BLK = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_BLOCK);
	this.rBLK = new RandomRange(-BLK, BLK);
	final int AGI = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_AGILITY);
	this.rAGI = new RandomRange(-AGI, AGI);
	final int VIT = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_VITALITY);
	this.rVIT = new RandomRange(-VIT, VIT);
	final int INT = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_INTELLIGENCE);
	this.rINT = new RandomRange(-INT, INT);
	final int LUC = this.personality.getAttribute(PersonalityConstants.PERSONALITY_ATTRIBUTE_RANDOM_LUCK);
	this.rLUC = new RandomRange(-LUC, LUC);
	this.mSTR = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_STRENGTH_MODIFIER);
	this.mBLK = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_BLOCK_MODIFIER);
	this.mAGI = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_AGILITY_MODIFIER);
	this.mVIT = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_VITALITY_MODIFIER);
	this.mINT = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_INTELLIGENCE_MODIFIER);
	this.mLUC = this.gender.getAttribute(GenderConstants.GENDER_ATTRIBUTE_LUCK_MODIFIER);
	this.setStrength(
	...
	return super.getAttack() + this.getWeaponPower()
		+ this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_BONUS_ATTACK);
    }

    @Override
    public int getDefense() {
	return super.getDefense() + this.getArmorBlock()
		+ this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_BONUS_DEFENSE);
    }
	...
	if (x == 1) {
	    ...
	    final int whole = this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_LEVEL_SPEED_WHOLE);
	    final int fraction = this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_LEVEL_SPEED_FRACTION);
	...
	this.offsetBlock(StatConstants.GAIN_BLOCK + this.race.getAttribute(RaceConstants.RACE_ATTRIBUTE_BLOCK_PER_LEVEL)
    ...
    public void loadPartyMember(final int newLevel, final int chp, final int cmp, final int newGold,
	    final long newExperience, final int bookID, final boolean[] known, final boolean pcflag) {
