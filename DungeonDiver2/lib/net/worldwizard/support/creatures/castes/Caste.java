...
public class Caste extends Identifiable {
	...
	if (cdata.length != CasteConstants.CASTE_ATTRIBUTE_COUNT) {
	    throw new IllegalArgumentException(
		    "Exactly " + CasteConstants.CASTE_ATTRIBUTE_COUNT + " attributes must be specified!");
	}
	this.casteID = cid;
	this.data = cdata;
    }
	...
	longHash = longHash.add(IDGenerator.computeStringLongHash(CasteConstants.CASTE_NAMES[this.casteID])
	...
	longHash = longHash.add(
		IDGenerator.computeStringLongHash(CasteConstants.CASTE_NAMES[casteID]).multiply(BigInteger.valueOf(2)));
	return "$" + longHash.toString(36).toUpperCase();
    }

    @Override
    public void dumpContentsToFile() throws IOException {
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "castes");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
