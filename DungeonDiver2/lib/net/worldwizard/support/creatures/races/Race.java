...
public class Race extends Identifiable {
	...
	longHash = longHash.add(IDGenerator.computeStringLongHash(RaceConstants.RACE_NAMES[this.raceID])
	...
	longHash = longHash.add(
		IDGenerator.computeStringLongHash(RaceConstants.RACE_NAMES[raceID]).multiply(BigInteger.valueOf(2)));
	return "$" + longHash.toString(36).toUpperCase();
    }

    @Override
    public void dumpContentsToFile() throws IOException {
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "races");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
	final XDataWriter writer = new XDataWriter(Support.getSystemVariables().getBasePath() + File.separator + "races"
