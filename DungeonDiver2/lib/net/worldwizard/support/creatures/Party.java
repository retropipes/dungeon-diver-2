...
public class Party {
	...
	for (final BattleCharacter battler : this.battlers) {
		...
		act0.addActionArg(new GameScriptEntryArgument(
			Math.max(1, (10 * battler.getTemplate().getLevel() - 1) / (this.activeNPCs + this.activePCs))));
    ...
    private PartyMember pickPartyMemberInternal(final String[] pickNames, final int current, final int number) {
	final String response = CommonDialogs.showInputDialog(
		"Pick " + number + " Party Member(s) - " + current + " of " + number, "DungeonDiverII", pickNames,
		pickNames[0]);
	if (response != null) {
	    final int loc = this.findMember(response, 0, this.playerLimit);
	    if (loc != -1) {
