...
public class SystemRegistration {
    ...
    // Methods
    public static String[] getRawSystemObjectList(final String type, final String plural) {
	// Configure type
	SystemRegistration.systemObjectType = type;
	SystemRegistration.systemObjectPluralType = plural;
	final ArrayList<String> registeredNames = SystemRegistration.readSystemObjectRegistry();
    ...
    public static String[] getSystemObjectNameList(final String type, final String plural) {
	// Load systemObject list
	final String[] rawSystemObjectList = SystemRegistration.getRawSystemObjectList(type, plural);
	if (rawSystemObjectList != null) {
		...
		systemObjectList[x] = rawSystemObjectList[x].split(",")[1].trim();
	    }
	    return systemObjectList;
	}
	return null;
    ...
    public static String[] getSystemObjectIDList(final String type, final String plural) {
	// Load systemObject list
	final String[] rawSystemObjectList = SystemRegistration.getRawSystemObjectList(type, plural);
	if (rawSystemObjectList != null) {
	    final String[] systemObjectList = new String[rawSystemObjectList.length];
	    for (int x = 0; x < rawSystemObjectList.length; x++) {
		systemObjectList[x] = rawSystemObjectList[x].split(",")[0].trim();
	    }
	    return systemObjectList;
	}
	return null;
    ...
    public static void autoregisterSystemObject(final String id, final String name, final String type,
	    final String plural) {
	// Load systemObject list
	final String[] systemObjectIDList = SystemRegistration.getSystemObjectIDList(type, plural);
	final String[] systemObjectNameList = SystemRegistration.getSystemObjectNameList(type, plural);
	if (id != null && name != null) {
		...
		if (systemObjectIDList != null && systemObjectNameList != null) {
			...
			if (x < systemObjectIDList.length) {
			    newSystemObjectList[x] = systemObjectIDList[x] + " , " + systemObjectNameList[x];
			} else {
			    newSystemObjectList[x] = id + " , " + name;
			}
		    }
		    SystemRegistration.writeSystemObjectRegistry(newSystemObjectList);
		} else {
		    SystemRegistration.writeSystemObjectRegistry(new String[] { id + " , " + name });
		}
	    }
	}
    }
	...
	try (final FileInputStream fis = new FileInputStream(basePath + SystemRegistration.systemObjectType + "Registry"
    ...
    private static void writeSystemObjectRegistry(final String[] newSystemObjectList) {
	final String basePath = SystemRegistration.getBasePath();
	// Check if registry is writable
	final File regFile = new File(basePath + SystemRegistration.systemObjectType + "Registry"
		+ Extension.getRegistryExtensionWithPeriod());
	if (!regFile.exists()) {
