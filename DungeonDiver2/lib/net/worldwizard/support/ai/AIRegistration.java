...
public class AIRegistration {
	...
	final ArrayList<String> registeredNames = AIRegistration.readAIRegistry();
	...
	if (res != null) {
		...
		if (new File(AIRegistration.getBasePath() + File.separator + res
			+ Extension.getAIScriptExtensionWithPeriod()).exists()) {
		    // Register it
	...
	// Load AI list
	final String[] aiList = AIRegistration.getAIList();
	final String[] aiNames = new File(AIRegistration.getBasePath()).list(new AIRegistrationFilter());
	if (aiNames != null && aiNames.length > 0) {
	    ...
	    // Pick AI to register
	    final String res = CommonDialogs.showInputDialog("Register Which AI?", "Register AI", aiNames, aiNames[0]);
	    if (res != null) {
		// Verify that AI is not already registered
		boolean alreadyRegistered = false;
		if (aiList != null) {
		...
		if (!alreadyRegistered) {
			...
			CommonDialogs.showDialog("The AI to register is not a valid AI.");
		    }
		} else {
		    CommonDialogs.showDialog("The AI to register has been registered already.");
		}
	    }
	} else {
	...
	final FileDialog fc = new FileDialog((Frame) null, "Load AI Script", FileDialog.LOAD);
	fc.setVisible(true);
	...
	// Check for null list
	if (aiList == null) {
	    CommonDialogs.showTitledDialog("No AIs Registered!", "Unregister AI");
	    return;
	}
	// Pick AI to unregister
	final String res = CommonDialogs.showInputDialog("Unregister Which AI?", "Unregister AI", aiList, aiList[0]);
	if (res != null) {
	    // Find AI index
	    int index = -1;
	...
	try (final FileInputStream fis = new FileInputStream(
		basePath + File.separator + "AIRegistry" + Extension.getRegistryExtensionWithPeriod());
