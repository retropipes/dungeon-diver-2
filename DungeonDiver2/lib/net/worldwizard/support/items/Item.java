...
public class Item extends Identifiable {
	...
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.initialUses).multiply(BigInteger.valueOf(4)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.weightPerUse).multiply(BigInteger.valueOf(5)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.buyPrice).multiply(BigInteger.valueOf(6)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.sellPrice).multiply(BigInteger.valueOf(7)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.weight).multiply(BigInteger.valueOf(8)));
	longHash = longHash.add(IDGenerator.computeLongLongHash(this.potency).multiply(BigInteger.valueOf(9)));
	longHash = longHash.add(IDGenerator.computeBooleanLongHash(this.combatUsable).multiply(BigInteger.TEN));
	return longHash;
    }

    @Override
    public void dumpContentsToFile() throws IOException {
	final File dir = new File(Support.getSystemVariables().getBasePath() + File.separator + "items");
	if (!dir.exists()) {
	    dir.mkdirs();
	}
