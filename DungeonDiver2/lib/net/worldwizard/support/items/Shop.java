...
public class Shop implements ShopTypes {
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
		...
		this.choices = EquipmentFactory.createOneHandedWeaponNames(playerCharacter.getCaste().getCasteID());
		// Choose Hand
		...
		this.choices = EquipmentFactory.createTwoHandedWeaponNames(playerCharacter.getCaste().getCasteID());
	    }
	} else if (this.type == ShopTypes.SHOP_TYPE_ARMOR) {
	    this.choices = EquipmentFactory.createArmorNames(this.typeIndex);
	} else if (this.type == ShopTypes.SHOP_TYPE_HEALER || this.type == ShopTypes.SHOP_TYPE_REGENERATOR) {
	...
	if (this.type == ShopTypes.SHOP_TYPE_HEALER
	    ...
	    return false;
	} else if (this.type == ShopTypes.SHOP_TYPE_SPELLS && playerCharacter.getSpellBook()
	...
	final PartyMember playerCharacter = PartyManager.getParty().getLeader();
	this.cost = 0;
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS || this.type == ShopTypes.SHOP_TYPE_ARMOR) {
	...
	if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
		...
		playerCharacter.getItems().equipOneHandedWeapon(bought, this.handIndex);
	    } else {
	    ...
	    playerCharacter.offsetGold(-this.cost);
	    final Equipment bought = EquipmentFactory.createArmor(this.index, this.typeIndex);
