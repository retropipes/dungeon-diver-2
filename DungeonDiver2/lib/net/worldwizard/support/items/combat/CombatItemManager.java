...
public class CombatItemManager {
    ...
    private static final Creature SOURCE = PartyManager.getNewPCInstance(0, 0, 0, 0, 0, "");
    ...
    public static boolean selectAndUseItem(final Creature user, final int teamID, final boolean aiEnabled,
	    final BattleDefinitions battle) {
	boolean result = false;
	final CombatUsableItem i = CombatItemManager.selectItem(user);
	if (i != null) {
	    result = CombatItemManager.useItem(i, user, teamID, aiEnabled, battle);
	}
	return result;
    }
    ...
    public static boolean useItem(final CombatUsableItem used, final Creature user, final int teamID,
	...
	if (used != null) {
	    ...
	    used.use();
	    e.resetEffect();
	    final Creature[] targets = CombatItemManager.resolveTarget(used, user, teamID, aiEnabled, battle);
    ...
    private static Creature[] resolveTarget(final CombatUsableItem used, final Creature user, final int teamID,
	    final boolean aiEnabled, final BattleDefinitions battle) {
